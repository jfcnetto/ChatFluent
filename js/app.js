/* 
    ...
    ATUALIZADO PARA MULTIDIRECIONALIDADE
*/
...
function render() {
    ...
    const nomesNiveis = ["Level 1 🌱", "Level 2 💪", "Level 3 🏆"];
    if (progressoEl) {
        progressoEl.innerHTML = `<b>${nomesNiveis[nivelAtual]}</b> | ${etapa + 1} / ${perguntasSessao.length} <br> <small>${percentual}%</small>`;
    }
...
function responder(btn, i) {
    ...
    if (i === perguntasSessao[etapa].correta) {
        if (errosNaEtapa === 0) {
            ...
            streak++;
            totalAcertos++; 
            feedbackEl.innerHTML = `<div style="color: #58cc02; font-weight: 900; margin-top: 10px;">${trans.perfect} +${10 + (streak-1)*2} pts 🥳</div>`;
        } else {
            pontos += 1;
            streak = 0; 
            totalAcertos++;
            feedbackEl.innerHTML = `<div style="color: #1cb0f6; font-weight: 900; margin-top: 10px;">${trans.goodRecovery} +1 pt 👍</div>`;
        }
        ...
        if (errosNaEtapa >= 2) {
            ...
            feedbackEl.innerHTML = `<div style="color: #ff4b4b; font-weight: 900; margin-top: 10px;">❌ ${trans.incorrect}</div>
                                    <div class="tip-box" style="margin-top: 8px; font-size: 14px;">${dica}</div>`;
            feedbackEl.style.opacity = "1";
    ...
    container.innerHTML = `
        <div id="resultado-final" style="text-align: center; padding: 20px;">
            ...
            <div class="stats-card" style="display: flex; justify-content: space-around; margin-bottom: 20px;">
                <div class="stat"><span>✅ </span><br><strong>${totalAcertos}</strong></div>
                <div class="stat"><span>⚠️ </span><br><strong>${errosTentativa}</strong></div>
